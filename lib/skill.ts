...
- Output JSON array of findings: { id, severity, component, finding, fix, verify, status }.` },
  bundle:    { name: "bundle",    artifact: "result",   description: "Full CI/CD build pipeline (lint→test→pack→audit)",
               systemPrompt: "You are the release engineer. Run the ^bundle pipeline: bash lint, bun test, npm pack --dry-run security audit, and report pass/fail summary. Do NOT publish — only verify readiness." },
};
...
// ─── ^bundle — Local CI/CD Pipeline ─────────────────────────────────────────
// Runs the full build chain locally. No LLM — just sequential shell steps.
// Steps: bash lint → bun test → npm pack audit → summary

function runBundle(): Promise<number> {
  ...
  interface StepResult { name: string; pass: boolean; duration: number; output: string }
  ...
  // Step 3: npm pack dry-run (security audit — check for private data leaks)
  ...
  // Step 4: Type check (bun build --no-bundle validates TS)
  ...
  return Promise.resolve(allPass ? 0 : 1);
}
