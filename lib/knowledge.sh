...
mode_knowledge() {
  ...
  # Rich terminal status views stay in bash (ANSI colors, aligned columns)
  if [[ "$sub_cmd" == "status" ]]; then
    case "$cmd_type" in
      task|workflow|project)
        knowledge_status "$cmd_type" "$args" ""
        return $?
        ;;
    esac
  fi

  # All other subcommands delegate to bun (canonical implementation)
  bun run "$AGENCE_LIB/commands.ts" "$cmd_type" "$sub_cmd" $args
  return $?
}
